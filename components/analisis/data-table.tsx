            ...
            <Card key={row.id}>
              <CardHeader>
                <CardTitle className="flex items-center justify-between text-lg">
                  <span>{(row.original as Doc<"Analisis">).nombre}</span>
                  <AnalisisActions analisis={row.original as Doc<"Analisis">} />
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                <div className="flex justify-between border-t pt-2">
                  <span className="text-sm text-muted-foreground">Tipo</span>
                  <span className="text-sm font-medium">{(row.original as Doc<"Analisis">).Tipo}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Registro</span>
                  <span className="text-sm font-medium">
                    {new Date((row.original as Doc<"Analisis">).fechaRegistro).toLocaleDateString()}
                  </span>
                </div>
              </CardContent>
            </Card>
